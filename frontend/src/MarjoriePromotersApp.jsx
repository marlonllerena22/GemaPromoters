...
function PromoterHome({ profile, setNotice, setView }) {
  ...
  return <>
    ...
    {profile.status === 'active' && <>
      ...
      <section className="mb-rate-ladder" aria-label="Niveles de comisión">{[['Inicial', '1–4 pares', '$2,50'], ['Plata', '5–9 pares', '$5'], ['Oro', '10–29 pares', '$7,50'], ['Platino', '30+ pares', '$10']].map(([level, pairs, rate]) => <article key={level} className={profile.level.name === level ? 'active' : ''}><span>{level}</span><strong>{rate}<small> por par</small></strong><small>{pairs}</small></article>)}</section>
...
function PromoterSales({ profile }) {
  return <section className="mb-member-section"><div className="mb-section-title"><span>ACTIVIDAD</span><h2>Mis ventas</h2><p>Solo las ventas con un total final de $65 o más suman pares, nivel y comisión.</p></div><div className="mb-member-list">{profile.sales.map((sale) => <article key={sale.id}><div><strong>{sale.customer_name}</strong><span>{sale.branch_name}</span><small>{date(sale.sale_date)} · {sale.pairs} pares · {sale.sale_total == null ? 'Total no registrado' : money(sale.sale_total)}</small></div><div><strong>{sale.effective_pairs} válidos</strong><span className={!sale.commission_eligible ? 'mb-sale-ineligible' : ''}>{sale.is_voided ? 'Anulada' : !sale.commission_eligible ? sale.eligibility_reason : sale.returned_pairs ? `${sale.returned_pairs} devueltos` : sale.is_paid && sale.is_delivered ? 'Confirmada' : 'En proceso'}</span></div></article>)}{!profile.sales.length && <p className="mb-empty">Todavía no tienes ventas registradas.</p>}</div></section>;
}
...
function AdminPromoterDetail({ detail, reload, close, permissions }) {
  const [notice, setNotice] = useState('');
  const [sale, setSale] = useState({ branch_client_id: '', customer_name: '', customer_whatsapp: '', pairs: 1, sale_total: '', sale_date: today(), is_paid: true, is_delivered: true, notes: '' });
  ...
  return <section className="mba-detail"><div className="mba-detail-top"><button className="mba-icon" onClick={close}><X /></button><div className="mba-person"><span>{detail.photo_url ? <img src={detail.photo_url} alt="" /> : <UserRound />}</span><div><small>{detail.code || 'SOLICITUD'}</small><h2>{detail.name}</h2><p>{detail.email} · {detail.whatsapp}</p></div></div><span className={`mb-status ${detail.status}`}>{detail.status_label}</span>{permissions.manage_promoters && detail.status === 'pending' && <button className="mb-primary" onClick={() => action(`/marjorie/admin/promoters/${detail.id}/approve`, { method: 'POST' }, (result) => result.email_sent ? 'Promotora aprobada. Enviamos sus accesos al correo registrado.' : `Promotora aprobada, pero el correo no se pudo enviar: ${result.email_reason || 'revisa la configuración'}`)}>Aprobar solicitud</button>}</div><Notice value={notice} error={/no |error|revisa|superar|debe/i.test(notice)} />
    ...
    {permissions.manage_promoters && <details className="mba-editor"><summary><Pencil /> Editar datos y estado</summary><form onSubmit={(e) => { e.preventDefault(); action(`/marjorie/admin/promoters/${detail.id}`, { method: 'PATCH', body: JSON.stringify(edit) }, 'Datos actualizados'); }}><div className="mb-form-grid"><label>Nombre<input value={edit.name} onChange={(e) => setEdit({ ...edit, name: e.target.value })} /></label><label>Cédula<input value={edit.cedula} onChange={(e) => setEdit({ ...edit, cedula: e.target.value })} /></label><label>WhatsApp<input value={edit.whatsapp} onChange={(e) => setEdit({ ...edit, whatsapp: e.target.value })} /></label><label>Correo<input value={edit.email} onChange={(e) => setEdit({ ...edit, email: e.target.value })} /></label><label>Instagram<input value={edit.instagram} onChange={(e) => setEdit({ ...edit, instagram: e.target.value })} /></label><label>Ciudad<input value={edit.city} onChange={(e) => setEdit({ ...edit, city: e.target.value })} /></label><label>Estado<select value={edit.status} onChange={(e) => setEdit({ ...edit, status: e.target.value })}>{['pending', 'active', 'review', 'suspended', 'revoked', 'rejected'].map((item) => <option key={item} value={item}>{statusText[item]}</option>)}</select></label><label>Observaciones<textarea value={edit.admin_notes || ''} onChange={(e) => setEdit({ ...edit, admin_notes: e.target.value })} /></label></div><button className="mb-primary"><Save /> Guardar cambios</button></form></details>}
    {detail.activated_at && <><section className="mba-work-grid">{permissions.manage_sales && <form className="mba-sale-form" onSubmit={addSale}><div className="mba-heading"><div><span>VENTA REFERIDA</span><h3>Registrar pares</h3></div></div><label>Local<select required value={sale.branch_client_id} onChange={(e) => setSale({ ...sale, branch_client_id: e.target.value })}><option value="">Seleccionar</option>{detail.branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label><label>Cliente<input value={sale.customer_name} onChange={(e) => setSale({ ...sale, customer_name: e.target.value })} /></label><label>WhatsApp cliente<input value={sale.customer_whatsapp} onChange={(e) => setSale({ ...sale, customer_whatsapp: e.target.value })} /></label><div className="mb-form-grid"><label>Pares<input min="1" type="number" value={sale.pairs} onChange={(e) => setSale({ ...sale, pairs: e.target.value })} /></label><label>Total final pagado ($)<input required min="0" step="0.01" inputMode="decimal" type="number" value={sale.sale_total} onChange={(e) => setSale({ ...sale, sale_total: e.target.value })} /><small>Desde $65 genera comisión.</small></label><label>Fecha<input type="date" max={today()} value={sale.sale_date} onChange={(e) => setSale({ ...sale, sale_date: e.target.value })} /></label></div><label className="mb-check"><input type="checkbox" checked={sale.is_paid} onChange={(e) => setSale({ ...sale, is_paid: e.target.checked })} />Pagada</label><label className="mb-check"><input type="checkbox" checked={sale.is_delivered} onChange={(e) => setSale({ ...sale, is_delivered: e.target.checked })} />Entregada</label><button className="mb-primary"><Plus /> Registrar venta</button></form>}{permissions.review_bonuses && <div className="mba-bonuses"><div className="mba-heading"><div><span>CICLO ACTUAL</span><h3>Bonos digitales</h3></div></div><BonusEditor key={`${detail.id}-1-${detail.bonuses.length}`} detail={detail} cut={1} save={saveBonus} /><BonusEditor key={`${detail.id}-2-${detail.bonuses.length}`} detail={detail} cut={2} save={saveBonus} /></div>}</section>
    <section className="mba-band"><div className="mba-heading"><div><span>MOVIMIENTOS</span><h3>Historial de ventas</h3></div>{permissions.manage_payments && detail.payable_cut?.due_date <= today() && detail.payable_total > 0 && <button className="mb-primary" onClick={() => action(`/marjorie/admin/promoters/${detail.id}/pay`, { method: 'POST', body: JSON.stringify({}) }, 'Pago registrado')}>Pagar corte {detail.payable_cut.cut_number}: {money(detail.payable_total)}</button>}</div><div className="mba-sales-list">{detail.sales.map((row) => <article key={row.id}><div><strong>{row.customer_name}</strong><span>{row.branch_name} · {date(row.sale_date)}</span><small>{row.pairs} pares · {row.effective_pairs} válidos · {row.sale_total == null ? 'Sin total' : money(row.sale_total)}{!row.commission_eligible ? ` · ${row.eligibility_reason}` : ''}</small></div>{permissions.manage_sales && <><label>Pagada<input type="checkbox" checked={Boolean(row.is_paid)} onChange={(e) => toggleSale(row, { is_paid: e.target.checked })} /></label><label>Entregada<input type="checkbox" checked={Boolean(row.is_delivered)} onChange={(e) => toggleSale(row, { is_delivered: e.target.checked })} /></label><label>Total $<input min="0" step="0.01" type="number" defaultValue={row.sale_total ?? ''} onBlur={(e) => toggleSale(row, { sale_total: e.target.value })} /></label><label>Devueltos<input min="0" max={row.pairs} type="number" defaultValue={row.returned_pairs} onBlur={(e) => toggleSale(row, { returned_pairs: e.target.value })} /></label><button className={row.is_voided ? 'mb-secondary' : 'mba-danger'} onClick={() => toggleSale(row, { is_voided: !row.is_voided })}>{row.is_voided ? 'Restaurar' : 'Anular'}</button></>}</article>)}{!detail.sales.length && <p className="mb-empty">No hay ventas registradas.</p>}</div></section></>}
  </section>;
}
...
export function MarjoriePromotersAdmin({ embedded = false, user = null, onLogout = null }) {
  const ownerPermissions = { manage_promoters: true, manage_content: true, review_bonuses: true, view_sales: true, manage_sales: true, manage_payments: true, manage_settings: true };
  const managerPermissions = { manage_promoters: false, manage_content: false, review_bonuses: false, view_sales: false, manage_sales: false, manage_payments: false, manage_settings: false };
  const permissions = user?.role === 'marjorie_admin' ? { ...managerPermissions, ...user.permissions } : ownerPermissions;
  const [view, setView] = useState('overview');
  const [dashboard, setDashboard] = useState(null);
  const [promoters, setPromoters] = useState([]);
  const [content, setContent] = useState({ library: [], requests: [] });
  const [settings, setSettings] = useState({ discount_percent: 0, support_whatsapp: '' });
  ...
  async function load() {
    try {
      const [nextDashboard, nextPromoters] = await Promise.all([api('/marjorie/admin/dashboard'), api('/marjorie/admin/promoters')]);
      setDashboard(nextDashboard);
      setPromoters(nextPromoters);
      if (permissions.manage_content) setContent(await api('/marjorie/admin/content'));
      if (permissions.manage_settings) setSettings(await api('/marjorie/admin/settings'));
      if (selectedId) setDetail(await api(`/marjorie/admin/promoters/${selectedId}`));
      setError('');
    } catch (err) { setError(err.message); }
  }
  useEffect(() => { load(); }, []);
  ...
  const nav = [
    ...
    ['promoters', 'Promotoras', UsersRound],
    ...(permissions.manage_content ? [['content', 'Contenido', ImageIcon]] : []),
    ...(permissions.manage_settings ? [['settings', 'Configuración', Settings]] : [])
  ];
  function openPromoter(id) { setSelectedId(id); setView('promoters'); }
  return <main className={`mba-app ${embedded ? 'embedded' : ''}`}><header className="mba-top"><div><img src={LOGO} alt="" /><span>{user?.name ? `${user.name} · Promotoras` : 'Administración de Promotoras'}</span></div><button className="mba-menu" onClick={() => setMenu(!menu)}><Menu /> Opciones</button><nav className={menu ? 'open' : ''}>{nav.map(([key, label, Icon]) => <button key={key} className={view === key ? 'active' : ''} onClick={() => { setView(key); setMenu(false); }}><Icon />{label}{key === 'promoters' && dashboard?.stats?.pending > 0 && <span>{dashboard.stats.pending}</span>}</button>)}{!embedded && onLogout && <button onClick={onLogout}><LogOut />Salir</button>}</nav></header><Notice value={error} error /><div className="mba-body">{view === 'overview' && <AdminOverview dashboard={dashboard} openPromoter={openPromoter} />}{view === 'promoters' && !detail && <AdminPromoters rows={promoters} selectedId={selectedId} setSelectedId={setSelectedId} />}{view === 'promoters' && detail && <AdminPromoterDetail key={`${detail.id}-${detail.updated_at}-${detail.sales.length}-${detail.bonuses.length}`} detail={detail} permissions={permissions} close={() => setSelectedId(null)} reload={async () => { await load(); setDetail(await api(`/marjorie/admin/promoters/${detail.id}`)); }} />}{view === 'content' && permissions.manage_content && <AdminContent data={content} reload={load} />}{view === 'settings' && permissions.manage_settings && <AdminSettings settings={settings} reload={load} />}</div></main>;
}
