...
import {
  ...
  DollarSign, ExternalLink, FileText, Gift, Home, Image as ImageIcon, KeyRound,
...
function AdminPromoterDetail({ detail, reload, close }) {
  ...
  async function action(path, options, message) { try { await api(path, options); await reload(); setNotice(message); } catch (err) { setNotice(err.message); } }
