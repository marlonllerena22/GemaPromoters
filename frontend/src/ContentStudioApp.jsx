      ...
      <div className="cs-user-list">
        ...
        {(data.users || []).map((item) => {
          ...
          return <article key={item.id} className={!active ? 'inactive' : ''}>
            ...
            <div className="cs-user-actions"><select aria-label={`Plan de ${item.name}`} value={userPackages[item.id] || PLAN_PACKAGES.find((plan) => plan.photos === Number(item.monthly_limit))?.id || 'emprendedor'} onChange={(event) => setUserPackages({ ...userPackages, [item.id]: event.target.value })}>{PLAN_PACKAGES.map((plan) => <option key={plan.id} value={plan.id}>{plan.name} · {plan.photos} fotos · ${Number(plan.price).toFixed(2)}</option>)}</select><button type="button" onClick={() => applyUserPlan(item)}>Aplicar plan</button><button type="button" className={Number(item.can_delete_generations) === 0 ? 'permission-off' : ''} onClick={() => updateUser(item, { can_delete_generations: Number(item.can_delete_generations) === 0 }, `${item.name} ${Number(item.can_delete_generations) === 0 ? 'ya puede eliminar sus imágenes' : 'ya no puede eliminar sus imágenes'}.`)}>{Number(item.can_delete_generations) === 0 ? 'Permitir eliminar fotos' : 'Bloquear eliminación'}</button><button type="button" onClick={() => updateUser(item, { status: item.status === 'active' ? 'inactive' : 'active' }, `${item.name} ${item.status === 'active' ? 'fue desactivado' : 'fue activado'}.`)}>{item.status === 'active' ? 'Desactivar' : 'Activar'}</button></div>
          </article>;
        })}
