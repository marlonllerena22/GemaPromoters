...
import {
  ...
  MapPin, PersonStanding, Phone, ScanSearch, RefreshCw, Info, Home, Bot
} from 'lucide-react';
...
import ContentStudioLumi from './ContentStudioLumi.jsx';
...
export default function ContentStudioApp({ user, onLogout, embedded = false, establishmentId, initialTab = 'create', sellerDemo = false }) {
  ...
  const navigation = isSellerWorkspace
    ...
    : [['create', 'Crear', Sparkles], ['history', 'Historial', LayoutGrid], ['lumi', 'Lumi', Bot], ['profile', 'Perfil', UserRound], ...(isStudioAdmin ? [['users', 'Usuarios', UsersRound]] : []), ['settings', 'Configuración', Settings]];
  ...
  return <div className={`cs-app ${embedded ? 'cs-embedded' : ''}`}>
    ...
    <div className="cs-page">
      ...
      {loading && !data ? <StudioShellSkeleton /> : data && <>
        ...
        {tab === 'lumi' && <ContentStudioLumi onUpgrade={(planId) => { setRequestedPlan(planId); setPlansOpen(true); }} setGlobalError={setError} />}
    ...
    {!embedded && <nav className={`cs-mobile-nav ${isStudioAdmin ? 'admin' : ''}`} style={{ gridTemplateColumns: `repeat(${navigation.length}, minmax(0, 1fr))` }} aria-label="Navegación principal">{navigation.map(([key,label,Icon]) => <button key={key} className={tab === key ? 'active' : ''} onClick={() => key === 'create' ? newCreation() : setTab(key)}><Icon size={21}/><span>{label}</span></button>)}</nav>}
...
function UsersView({ data, scopeBody, reload, setError }) {
  ...
  async function applyUserPlan(item) {
    ...
    await updateUser(item, {
      ...
      lumi_enabled: Boolean(currentPlan.lumi),
      status: 'active'
      ...
      <div className="cs-user-list">
        ...
        {(data.users || []).map((item) => {
          ...
          return <article key={item.id} className={!active ? 'inactive' : ''}>
            ...
            <div className="cs-user-actions"><select aria-label={`Plan de ${item.name}`} value={userPackages[item.id] || (Number(item.lumi_enabled) === 1 ? 'lumi' : PLAN_PACKAGES.find((plan) => plan.photos === Number(item.monthly_limit) && !plan.lumi)?.id) || 'emprendedor'} onChange={(event) => setUserPackages({ ...userPackages, [item.id]: event.target.value })}>{PLAN_PACKAGES.map((plan) => <option key={plan.id} value={plan.id}>{plan.name} · {plan.photos} fotos · ${Number(plan.price).toFixed(2)}</option>)}</select><button type="button" onClick={() => applyUserPlan(item)}>Aplicar plan</button><button type="button" className={Number(item.lumi_enabled) === 1 ? '' : 'permission-off'} onClick={() => updateUser(item, { lumi_enabled: Number(item.lumi_enabled) !== 1 }, `${item.name} ${Number(item.lumi_enabled) === 1 ? 'ya no tiene Lumi Business' : 'ya tiene Lumi Business'}.`)}>{Number(item.lumi_enabled) === 1 ? 'Quitar Lumi' : 'Activar Lumi'}</button><button type="button" className={Number(item.can_delete_generations) === 0 ? 'permission-off' : ''} onClick={() => updateUser(item, { can_delete_generations: Number(item.can_delete_generations) === 0 }, `${item.name} ${Number(item.can_delete_generations) === 0 ? 'ya puede eliminar sus imágenes' : 'ya no puede eliminar sus imágenes'}.`)}>{Number(item.can_delete_generations) === 0 ? 'Permitir eliminar fotos' : 'Bloquear eliminación'}</button><button type="button" onClick={() => updateUser(item, { status: item.status === 'active' ? 'inactive' : 'active' }, `${item.name} ${item.status === 'active' ? 'fue desactivado' : 'fue activado'}.`)}>{item.status === 'active' ? 'Desactivar' : 'Activar'}</button></div>
          </article>;
        })}
