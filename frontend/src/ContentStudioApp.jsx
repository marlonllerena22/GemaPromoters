...
const emptyForm = { preset: 'editorial', editorial_subject: 'female', logo_id: 'none', include_contact: false, social_format: 'post', social_framing: 'smart', social_style: 'editorial', product_name: '', product_features: '', creative_instruction: '' };
const PLAN_PACKAGES = [
      ...
      <div className="cs-workspace">
        <div className="cs-main-column">
          ...
          <section className="cs-card cs-content-card">
            ...
            {form.preset === 'social' && <div className="cs-social-options"><div><strong>Formato</strong><div className="cs-choice-row">{(data.social_formats || []).map((format) => <button type="button" key={format.id} className={form.social_format === format.id ? 'selected' : ''} onClick={() => setForm({ ...form, social_format: format.id })}><span>{format.id === 'landscape' ? '▰' : format.id === 'story' ? '▯' : '▣'}</span><div><b>{format.label}</b><small>{format.width} × {format.height}</small></div><Check size={16} /></button>)}</div></div><div><strong>Cómo encuadrar tu post</strong><div className="cs-social-framing-grid">{(data.social_framings || []).map((framing) => <button type="button" key={framing.id} className={form.social_framing === framing.id ? 'selected' : ''} onClick={() => setForm({ ...form, social_framing: framing.id })}><div><b>{framing.label}</b><small>{framing.description}</small></div><Check size={16} /></button>)}</div></div><div><strong>Estilo del diseño</strong><div className="cs-social-style-grid">{(data.social_styles || []).map((style) => <button type="button" key={style.id} className={form.social_style === style.id ? 'selected' : ''} onClick={() => setForm({ ...form, social_style: style.id })}><b>{style.name}</b><small>{style.description}</small>{form.social_style === style.id && <Check size={16} />}</button>)}</div></div></div>}
          </section>
